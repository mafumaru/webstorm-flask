...
class WordbaseHelper(BaseHelper):
    ...
    def get_groups_view_proxy(self):
        r = requests.post('http://' + server_ip + '/module/wordbase/helper/WordbaseHelper/get_groups_view')
        return r.text
    ...
    def delete_groups_proxy(self,id):
        r = requests.post('http://'+server_ip+'/module/wordbase/helper/WordbaseHelper/delete_groups?id='+id)
    ...
    def add_group_proxy(self,group_name):
        r = requests.post('http://' + server_ip + '/module/wordbase/helper/WordbaseHelper/add_group?group_name='+group_name)
        return r.text
    ...
    def tag(self,**args): # word_type , word_id , group_name
        if args['word_type'] == 'expression':
            ...
            row['wordset'][index]['group'].append(args['group_name'])

            self.db['wordset'].update({'wordset_id': row['wordset_id']}, {'$set': {'wordset': row['wordset']}})

    def remove_tag(self,**args):# word_type , word_id , group_id
        pass

    def filter_search(self,args):# * group -list, * wordbase_collection , * date(最近) , * wordt_type , keyword | offset , limit
        condition = args['condition_value']
        # if args['group']:
        #     condition = args['condition_value']
        #     # condition= {$elemMatch:{$elemMatch:}}
        # elif args['wordbase_collection']:
        #     condition= args['condition_value']
        # elif args['wordt_type']:
        #     condition= args['condition_value']
        # else:
        #     condition= None
        wordset_condition = {
            'wordset.' + args['condition_type']: condition
        }
        expression_condition = {args['condition_type']: condition}
        sentence_condition = {args['condition_type']: condition}
        wordset_result = self.db[self.collection['wordset']].find(wordset_condition)
        expression_result = self.db[self.collection['expression']].find(expression_condition)
        sentence_result = self.db[self.collection['sentence']].find(sentence_condition)
        res = self._to_list(wordset_result) + self._to_list(expression_result) + self._to_list(sentence_result)
        res.sort(key=lambda e: e['date'], reverse=True)
        return res
    ...
    def search_view_proxy(self,**args):# search(args)
        r = requests.post(
            'http://' + server_ip + '/module/wordbase/helper/WordbaseHelper/search_view',data=args)
        return r.text
...
class VideoHelper(BaseHelper):
    db_name = 'wordbase'
    video_history_collection_name = 'video_history'

    def __init__(self):
        super().__init__()
        self.video_history_collection = self.db[self.video_history_collection_name]

    def get_history(self,**args): # video_name
        history = self.video_history_collection.find_one({'video_name': args['video_name']})
        if history:
    ...
    def get_history_proxy(self,**args): # video_name
        r = requests.post('http://' + server_ip + '/module/wordbase/helper/VideoHelper/get_history',data=args)
        return r.text

    def set_history(self,**args):# video_name,history
        history = {
            'video_name':args['video_name'],
            'history':args['history'],
        }
        self.video_history_collection.remove({'video_name':args['video_name']})
        self.video_history_collection.insert(history)

    def set_history_proxy(self,**args):# video_name,history
        r = requests.post('http://' + server_ip + '/module/wordbase/helper/VideoHelper/set_history',data=args)